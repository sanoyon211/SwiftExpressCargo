...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pt-16 flex transition-colors duration-300">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-16 left-0 h-[calc(100vh-4rem)] w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 transform transition-transform duration-300 z-50 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full px-4 py-6 overflow-y-auto">
          <div className="mb-8 px-4">
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-50">User Portal</h2>
              ...
              <Link
                ...
                className={`flex items-center px-4 py-3 rounded-xl transition-colors ${
                  pathname === item.path
                    ...
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800'
                }`}
              >
          ...
          <div className="absolute bottom-8 w-[calc(100%-2rem)]">
            <button className="flex items-center px-4 py-3 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-colors w-full">
      ...
      <div className="flex-1 md:ml-64 w-full">
        <header className="bg-white dark:bg-slate-900 shadow-sm border-b border-slate-200 dark:border-slate-700 py-4 px-4 sm:px-6 md:hidden">
          <button
